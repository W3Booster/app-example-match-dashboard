...
import { matchupFor, matchupKey, matchupProblem, matchups } from '../src/notebook-model.ts';
...
test('AI opponents use the same matchup keys; missing data has a specific explanation', () => {
  ...
  assert.equal(matchupFor({ ...computer, match: { ...computer.match, map: '' } }, '0').map, '');
...
test('unsupported or incomplete matches cannot create misleading notes', () => {
  ...
  for (const match of [{ ...state.match, status: 'none' }, { ...state.match, mode: '2v2' }]) assert.equal(matchupFor({ ...state, match }, '0'), undefined);
...
test('all 16 directional matchups include mirrors and support general notes', () => {
  assert.equal(matchups.length, 16);
  assert.equal(new Set(matchups.map(matchupKey)).size, 16);
  assert.equal(matchups.filter(m => m.ownRace === m.opponentRace).length, 4);
  for (const matchup of matchups) {
    assert.equal(matchup.map, '');
    assert.notEqual(matchupKey(matchup), matchupKey({ ...matchup, map: 'Echo Isles' }));
  }
});
